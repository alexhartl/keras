...
from keras.src.callbacks.memory_usage_callback import running_on_gpu
from keras.src.callbacks.memory_usage_callback import running_on_tpu
...
class TestMemoryUsageCallback:
    ...
    def test_cpu_only_epoch_logging(self):
        ...
        with monkey:
            ...
            start = [
                ln
                for ln in lines
                if re.match(r"^Epoch \d+ start - CPU Memory:", ln)
            ]
            end = [
                ln
                for ln in lines
                if re.match(r"^Epoch \d+ end - CPU Memory:", ln)
            ]
            assert len(start) == self.epochs
            assert len(end) == self.epochs
            assert not any(
                "GPU Memory" in ln or "TPU Memory" in ln for ln in lines
            )
    ...
    def test_log_every_batch(self):
        ...
        with monkey:
            ...
            batches = [
                ln
                for ln in lines
                if re.match(r"^Batch \d+ end - CPU Memory:", ln)
            ]
    ...
    def test_get_gpu_memory_torch(self):
        ...
        fake_torch.cuda.max_memory_reserved.side_effect = [
            100 * 1024**2,
            200 * 1024**2,
        ]
        ...
        with (
            ...
            patch.dict(
                sys.modules,
                {"torch": fake_torch, "torch.cuda": fake_torch.cuda},
            ),
        ):
            cb = MemoryUsageCallback()
